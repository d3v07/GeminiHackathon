...
async function recallEncounterContext(agentId, role, otherRole, lat, lng) {
    ...
    try {
        ...
        console.warn(`[Encounter] Personal memory recall failed for ${agentId}:`, e.message);
    }
    ...
    try {
        ...
        console.warn(`[Encounter] Knowledge recall failed:`, e.message);
    }
    ...
    try {
        ...
        console.warn(`[Encounter] Cross-agent recall failed:`, e.message);
    }
...
async function handleEncounter(agentA, agentB) {
    ...
    console.log(`\n=== MULTI-TURN ENCOUNTER (#36) ===`);
    console.log(`${agentA.role} (${agentAId}) meets ${agentB.role} (${agentBId})`);
    console.log(`Location: ${agentA.lat}, ${agentA.lng}`);
    ...
    try {
        ...
        console.warn('[Encounter] Social context fetch failed:', e.message);
    }
        ...
        for (let si = 0; si < 2; si++) {
            ...
            try {
                ...
                if (text) {
                    ...
                    console.log(`  [Turn ${turn}] ${speaker.role}: "${text.substring(0, 100)}..."`);
                }
            } catch (e) {
                console.error(`  [Turn ${turn}] ${speaker.role} failed:`, e.message);
                dialogue.push({
                    speaker: speaker.role,
                    agentId: speaker.agentId || `unknown-${si === 0 ? 'a' : 'b'}`,
    ...
    try {
        ...
        console.log(`[Redis] Encounter cached: ${encounterKey}`);
    } catch (e) {
        console.warn('[Redis] Encounter cache failed:', e.message);
    }
