...
const logger = require('./logger').child({ module: 'rag-ingest' });
...
async function ingestKnowledgeBase(documents = NYC_KNOWLEDGE) {
    logger.info({ count: documents.length }, 'RAG ingestion pipeline started');
    ...
    for (const doc of documents) {
        ...
        for (const chunk of chunks) {
            try {
                ...
                ingested++;
                logger.info({ id, chunk: chunk.substring(0, 60) }, 'Document ingested');
            } catch (e) {
                failed++;
                logger.error({ chunk: chunk.substring(0, 60), err: e.message }, 'Document ingestion failed');
            }
        }
    }

    logger.info({ total: ingested + failed, ingested, failed }, 'RAG ingestion complete');
...
if (require.main === module) {
    ...
    if (fileIdx !== -1 && args[fileIdx + 1]) {
        ingestFromFile(args[fileIdx + 1])
            .then(() => process.exit(0))
            .catch(e => { logger.error({ err: e }, 'Ingestion failed'); process.exit(1); });
    } else {
        ingestKnowledgeBase()
            .then(() => process.exit(0))
            .catch(e => { logger.error({ err: e }, 'Ingestion failed'); process.exit(1); });
