...
async function perceiveNode(state, config) {
    ...
    try {
        ...
        console.warn(`[${npcId}] Routine injection failed:`, e.message);
    }
    ...
    try {
        ...
        console.warn(`[${npcId}] Memory recall failed:`, e.message);
    }
    ...
    try {
        ...
        console.warn(`[${npcId}] Knowledge recall failed:`, e.message);
    }
...
async function thinkNode(state, config) {
    ...
    try {
        ...
        console.error(`[${npcId}] Think error:`, e.message);
...
async function actNode(state, config) {
    ...
    for (const call of pendingToolCalls) {
        try {
            ...
            if (!validation.valid) {
                console.warn(`[${npcId}] Tool ${call.name} validation failed, using raw output`);
            }
    ...
    try {
        ...
        if (text.length > 20) {
            await storeMemory(npcId, text.substring(0, 500), {
                type: 'observation',
                lat, lng,
            }).catch(e => console.warn(`[${npcId}] Memory store failed:`, e.message));
        }
        ...
        console.error(`[${npcId}] Reflect error:`, e.message);
