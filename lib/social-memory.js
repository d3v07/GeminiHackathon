...
async function recordEncounter(agentA, agentB, { summary, sentiment = 0, location = {} } = {}) {
    ...
    try {
        ...
        console.log(`[Social Memory] ${agentA} ↔ ${agentB}: encounter #${encounterCount} (sentiment: ${sentiment.toFixed(2)})`);
        return { encounterCount, averageSentiment };
    } catch (e) {
        console.error(`[Social Memory] Failed to record encounter:`, e.message);
...
async function getRelationship(agentA, agentB) {
    ...
    try {
        ...
        console.error(`[Social Memory] Failed to get relationship:`, e.message);
        return null;
    }
}
...
async function getAgentRelationships(agentId) {
    try {
        ...
        console.error(`[Social Memory] Failed to get agent relationships:`, e.message);
        return [];
    }
}
