...
function init() {
    ...
    try {
        ...
        console.log('[Telemetry] Pub/Sub publisher initialized');
    } catch (e) {
        console.warn('[Telemetry] Pub/Sub unavailable, falling back to logs:', e.message);
        _disabled = true;
    }
}
...
async function publishTelemetry(agentId, state, action, meta = {}) {
    ...
    if (_disabled || !_topic) {
        ...
        if (_buffer.length >= BATCH_SIZE) {
            console.log(`[Telemetry] Local buffer flush (${_buffer.length} events)`);
            _buffer = [];
        }
        return event;
    }
    ...
    try {
        ...
        console.warn('[Telemetry] Pub/Sub publish failed:', e.message);
        _buffer.push(event);
    }
    ...
    if (_topic && _buffer.length > 0) {
        const promises = _buffer.map(evt =>
            _topic.publishMessage({ data: Buffer.from(JSON.stringify(evt)) })
                .catch(e => console.warn('[Telemetry] Flush failed:', e.message))
        );
